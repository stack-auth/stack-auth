...
const evalStepRun = table(
  { name: 'eval_step_run', public: true },
  {
    ...
    // Cumulative token usage reported by Claude Code's final `result` message.
    inputTokens: t.u64().optional(),
    outputTokens: t.u64().optional(),
    cacheReadTokens: t.u64().optional(),
    cacheCreationTokens: t.u64().optional(),
...
const spacetimedb = schema({ mcpCallLog, aiQueryLog, operators, qaEntries, evalWorkflow, evalRun, evalStepRun, evalWorklog, evalArtifact });
    ...
    for (const row of ctx.db.evalStepRun.iter()) {
      if (row.stepRunId === args.stepRunId) {
        ...
        ctx.db.evalStepRun.insert({
          ...
          cacheReadTokens: args.cacheReadTokens ?? row.cacheReadTokens,
          cacheCreationTokens: args.cacheCreationTokens ?? row.cacheCreationTokens,
