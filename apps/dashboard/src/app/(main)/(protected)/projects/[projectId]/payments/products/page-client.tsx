...
export default function PageClient() {
  ...
  const [shouldShowNewProduct, setShouldShowNewProduct] = useState(false);
  ...
  if (hasNoProductsAndNoItems) {
    return (
      <WelcomeScreen
        onCreateProduct={() => {
          // Switch to catalogs view and signal to create a new product
          setView("catalogs");
          setShouldShowNewProduct(true);
        }}
      />
    );
  }

  // Reset the flag when switching views
  const handleViewChange = (newView: "list" | "catalogs") => {
    setView(newView);
    setShouldShowNewProduct(false);
  };

  if (view === "catalogs") {
    return <PageClientCatalogsView onViewChange={handleViewChange} shouldCreateNewProduct={shouldShowNewProduct} />;
  }
  return <PageClientListView onViewChange={handleViewChange} />;
}
