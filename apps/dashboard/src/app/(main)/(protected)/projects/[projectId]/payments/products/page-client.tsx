...
export default function PageClient() {
  ...
  const [createFirstProduct, setCreateFirstProduct] = useState(false);
  ...
  // Handler for create product button from welcome screen
  const handleCreateFirstProduct = () => {
    setView("catalogs");
    setCreateFirstProduct(true);
  };

  // Show welcome screen if no products and items
  if (hasNoProductsAndNoItems && !createFirstProduct) {
    return <WelcomeScreen onCreateProduct={handleCreateFirstProduct} />;
  }

  if (view === "catalogs") {
    return <PageClientCatalogsView onViewChange={setView} createFirstProduct={createFirstProduct} onCreateFirstProductHandled={() => setCreateFirstProduct(false)} />;
  }
