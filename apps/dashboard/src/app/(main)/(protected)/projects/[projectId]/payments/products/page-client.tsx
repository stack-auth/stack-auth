...
export default function PageClient() {
  ...
  // Refs to communicate with child components
  const catalogViewRef = useRef<any>(null);
  ...
  const handleWelcomeCreateProduct = useCallback(() => {
    // Switch to catalog view if not already there
    setView("catalogs");
    // Tell the catalog view to create a new draft product
    setTimeout(() => {
      if (catalogViewRef.current?.createDraftProduct) {
        catalogViewRef.current.createDraftProduct();
      }
    }, 0);
  }, []);
  ...
  if (hasNoProductsAndNoItems) {
    return <WelcomeScreen onCreateProduct={handleWelcomeCreateProduct} />;
  }

  if (view === "catalogs") {
    return <PageClientCatalogsView ref={catalogViewRef} onViewChange={setView} />;
  }
