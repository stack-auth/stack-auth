...
type CatalogViewProps = {
  ...
  createDraftTrigger?: number,
};

function CatalogView({ groupedProducts, groups, existingItems, onSaveProduct, onDeleteProduct, onCreateNewItem, onOpenProductDetails, onSaveProductWithGroup, createDraftTrigger }: CatalogViewProps) {
  ...
  const lastCreateDraftTrigger = useRef<number | undefined>(undefined);
  ...
  useEffect(() => {
    if (createDraftTrigger === undefined) return;
    if (lastCreateDraftTrigger.current === createDraftTrigger) return;
    lastCreateDraftTrigger.current = createDraftTrigger;

    const targetType: 'user' | 'team' | 'custom' = 'user';
    setActiveType(targetType);

    const baseId = 'product';
    let key = baseId;
    let suffix = 2;
...
export default function PageClient({ onViewChange, createDraftTrigger }: { onViewChange: (view: "list" | "catalogs") => void, createDraftTrigger?: number }) {
    ...
    >
      <div className="flex-1">
        <CatalogView
          ...
          createDraftTrigger={createDraftTrigger}
        />
      </div>
    </PageLayout>
  );
