import { createBulldozerExecutionContext, toQueryableSqlQuery } from "../lib/bulldozer/db";
import { quoteSqlStringLiteral } from "../lib/bulldozer/db/utilities";
import { paymentsSchema } from "./schema/singleton";
import { queryRaw } from "../db";
import { REFUND_TXN_PREFIX, parseRefundTxnId } from "./refund-txn-id";
import { type Transaction, type TransactionEntry, type TransactionType } from "@hexclave/shared/dist/interface/crud/transactions";
...
async function getTransactions(options: {
  ...
  const rawRows = await queryRaw<Array<{ rowData: unknown }>>(sql);
  ...
  if (pageSourceRows.length > 0) {
    ...
    `;
    refundRows = await queryRaw<Array<{ rowData: unknown }>>(refundSql);
  }
