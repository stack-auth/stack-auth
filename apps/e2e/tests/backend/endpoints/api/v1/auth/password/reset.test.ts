...
async function getResetCode() {
  ...
  `);
  const resetCodeMessage = messages.find((m) => m.subject === "Mock subject, <Subject value={\"Reset your password at \" + project.displayName} />") ?? throwErr("Reset code message not found");
