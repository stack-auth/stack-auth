...
import type { TabKey } from "./streams";
...
export type ReplayState = {
  ...
  chunkRangesByTab: Map<TabKey, Array<ChunkRange>>,
...
function playEffectsForAllTabs(state: ReplayState, globalOffsetMs: number): Array<ReplayEffect> {
  const effects: Array<ReplayEffect> = [];
  for (const s of state.streams) {
...
export function replayReducer(state: ReplayState, action: ReplayAction): ReducerResult {
  switch (action.type) {
    ...
    case "SELECT_TAB": {
      const effects: Array<ReplayEffect> = [{ type: "pause_all" }];
