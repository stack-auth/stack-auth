...
export const emailTemplateCrudHandlers = createLazyProxy(() => createCrudHandlers(emailTemplateCrud, {
  ...
  async onRead({ params, auth }) {
    ...
    const emailTemplate = await oldDeprecatedPrismaClient.emailTemplate.findUnique({
      where: {
        projectId_type: {
  ...
  async onUpdate({ auth, data, params }) {
    ...
    const dbType = typedToUppercase(params.type);
    const oldTemplate = await oldDeprecatedPrismaClient.emailTemplate.findUnique({
      where: {
        projectId_type: {
          projectId: auth.tenancy.project.id,
          type: dbType,
    ...
    const db = await oldDeprecatedPrismaClient.emailTemplate.upsert({
      where: {
        projectId_type: {
          projectId: auth.tenancy.project.id,
          type: dbType,
  ...
  async onDelete({ auth, params }) {
    ...
    await oldDeprecatedPrismaClient.emailTemplate.delete({
      where: {
        projectId_type: {
          projectId: auth.tenancy.project.id,
          type: dbType,
  ...
  async onList({ auth }) {
    const templates = await oldDeprecatedPrismaClient.emailTemplate.findMany({
      where: {
        projectId: auth.tenancy.project.id,
      },
    });
