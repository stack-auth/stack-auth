...
export const teamsCrudHandlers = createLazyProxy(() => createCrudHandlers(teamsCrud, {
  ...
  onCreate: async ({ query, auth, data }) => {
    ...
    const db = await retryTransaction(getPrismaClientForSourceOfTruth(auth.tenancy.completeConfig.sourceOfTruth), async (tx) => {
  ...
  onRead: async ({ params, auth }) => {
    if (auth.type === 'client') {
      await ensureTeamMembershipExists(getPrismaClientForSourceOfTruth(auth.tenancy.completeConfig.sourceOfTruth), {
        tenancyId: auth.tenancy.id,
    ...
    const db = await getPrismaClientForSourceOfTruth(auth.tenancy.completeConfig.sourceOfTruth).team.findUnique({
      where: {
        tenancyId_teamId: {
          tenancyId: auth.tenancy.id,
          teamId: params.team_id,
  ...
  onList: async ({ query, auth }) => {
    ...
    const db = await getPrismaClientForSourceOfTruth(auth.tenancy.completeConfig.sourceOfTruth).team.findMany({
      where: {
        tenancyId: auth.tenancy.id,
        ...query.user_id ? {
          teamMembers: {
