...
export const teamsCrudHandlers = createLazyProxy(() => createCrudHandlers(teamsCrud, {
  ...
  onCreate: async ({ query, auth, data }) => {
    ...
    const db = await retryTransaction(oldDeprecatedPrismaClient, async (tx) => {
  ...
  onRead: async ({ params, auth }) => {
    if (auth.type === 'client') {
      await ensureTeamMembershipExists(oldDeprecatedPrismaClient, {
        tenancyId: auth.tenancy.id,
    ...
    const db = await oldDeprecatedPrismaClient.team.findUnique({
      where: {
        tenancyId_teamId: {
          tenancyId: auth.tenancy.id,
          teamId: params.team_id,
  ...
  onList: async ({ query, auth }) => {
    ...
    const db = await oldDeprecatedPrismaClient.team.findMany({
      where: {
        tenancyId: auth.tenancy.id,
        ...query.user_id ? {
          teamMembers: {
