...
import { failGrowthAgentPhase } from "@/lib/growth/agent-writes";
...
export const POST = createSmartRouteHandler({
  ...
  request: yupObject({
    ...
    body: yupObject({
      ...
      error_message: yupString().defined(),
    }).defined(),
  }),
  response: yupObject({
  ...
  handler: async ({ headers, params, body }) => {
    ...
    await failGrowthAgentPhase(
      { tenancy, runId: params.run_id, phaseKey: params.phase_key, attempt: body.attempt },
      { errorMessage: body.error_message },
    );
    return { statusCode: 200, bodyType: "json", body: { status: "failed" } };
  },
});
