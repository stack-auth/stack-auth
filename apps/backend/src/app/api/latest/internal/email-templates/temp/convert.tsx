...
export function generateTsxSourceFromConfiguration(configuration: TEditorConfiguration, variables: string[]): string {
...
${indent}</Container>`;
  ...
  return `import { Button, Container, Img } from "@react-email/components";

export function EmailTemplate({ ${propsInterface} }) {
  return (
${componentBody}
  );
}`;
...
export function getTransformedTemplateMetadata(metadata: EmailTemplateMetadata) {
  ...
  return {
    ...
    tsxSource: generateTsxSourceFromConfiguration(configuration, variableNames),
  };
}
