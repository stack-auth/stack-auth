import { splitConversation } from "@/lib/comms/conversations";
import { getPrismaClientForTenancy, retryTransaction } from "@/prisma-client";
import { createSmartRouteHandler } from "@/route-handlers/smart-route-handler";
import {
  commsConversationSchema,
  commsSplitConversationRequestSchema,
...
export const POST = createSmartRouteHandler({
  metadata: {
    summary: "Split conversation",
    description: "Moves the given messages from this conversation into a new conversation. Idempotent via idempotency_key.",
    tags: ["Comms"],
  },
  request: yupObject({
    auth: yupObject({
    ...
    body: commsSplitConversationRequestSchema.omit(["source_conversation_id"]),
  ...
  handler: async ({ auth, params, body }) => {
    ...
    const result = await retryTransaction(prisma, async (tx) => {
      return await splitConversation(tx, {
