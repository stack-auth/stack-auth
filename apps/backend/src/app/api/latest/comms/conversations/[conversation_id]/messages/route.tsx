import { listMessagesForConversation } from "@/lib/comms/messages";
...
export const GET = createSmartRouteHandler({
  metadata: {
    ...
    description: "Lists messages assigned to this conversation using an opaque cursor.",
    tags: ["Comms"],
  },
  request: yupObject({
    auth: yupObject({
  ...
  handler: async ({ auth, params, query }) => {
    ...
    const result = await retryTransaction(prisma, async (tx) => {
      return await listMessagesForConversation(tx, {
