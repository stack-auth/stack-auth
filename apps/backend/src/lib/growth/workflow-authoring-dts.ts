/**
 * The `@hexclave/workflows` type contract, served to the growth agent by the
 * workflow-authoring-context endpoint.
 *
 * This is a DELIBERATE COPY of the dashboard's editor DTS
 * (apps/dashboard/src/app/(main)/(protected)/projects/[projectId]/workflows/editor-types.ts), not a
 * shared module. An earlier draft hoisted that file into @hexclave/shared so both surfaces could
 * import one copy, but that meant a growth feature editing a file the workflows app owns. Growth is
 * a bolt-on: it must not perturb the workflows app, and deleting growth must not leave anything
 * behind in it. A private copy buys both — the workflows app is untouched, and removing growth is
 * `rm` on this directory.
 *
 * The price is drift. The dashboard copy is the source of truth; this one only feeds an LLM, so a
 * stale copy degrades generated workflows rather than breaking the customer-facing editor. The
 * likeliest drift — someone adds a platform event without updating this file — is caught by
 * workflow-authoring-dts.test.ts, which pins this map against the real catalog in
 * @hexclave/shared/interface/workflows. Payload FIELD changes are not caught by anything; if you
 * change a webhook event's payload, update this file too.
 *
 * Why a string rather than a real .d.ts: `@hexclave/workflows` does not exist on disk. It is a
 * virtual module inlined into every workflow bundle at sync time (see
 * lib/workflows/runtime-source.tsx), so there is no package for TypeScript to resolve and no
 * generated declaration to point at. Both consumers have to be handed the contract as text.
 */

export const GROWTH_WORKFLOWS_EDITOR_DTS = `
...
export const GROWTH_WORKFLOWS_EDITOR_AMBIENT_DTS = `declare module "date-fns";\ndeclare module "date-fns/*";`;
