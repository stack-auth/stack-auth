...
import { userFullInclude, userPrismaToCrud } from "@/app/api/latest/users/crud";
...
// Extracted verbatim from app/api/latest/internal/metrics/route.tsx so the loaders can be shared
// with non-route callers (growth metrics registry, benchmarks). The route file now only owns the
// request/response schemas and handler. Behavior must stay identical to the pre-extraction code.
...
export async function loadUsersByCountry(tenancy: Tenancy, now: Date, includeAnonymous: boolean = false): Promise<Record<string, number>> {
...
export async function loadActiveUsersByCountry(
  tenancy: Tenancy,
  now: Date,
...
// `$page-view` events).
export async function loadLiveUsersCount(
  tenancy: Tenancy,
  now: Date,
  includeAnonymous: boolean = false,
): Promise<number> {
...
export async function loadHourlyUsers(tenancy: Tenancy, now: Date, includeAnonymous: boolean = false): Promise<DataPoints> {
...
export async function loadDailyActiveUsers(tenancy: Tenancy, now: Date, includeAnonymous: boolean = false) {
  const todayUtc = new Date(now);
  todayUtc.setUTCHours(0, 0, 0, 0);
...
export async function loadHourlyActiveUsers(tenancy: Tenancy, now: Date, includeAnonymous: boolean = false): Promise<DataPoints> {
  const latestHour = new Date(now);
  latestHour.setUTCMinutes(0, 0, 0);
  const since = new Date(latestHour.getTime() - 23 * 60 * 60 * 1000);
  const untilExclusive = new Date(latestHour.getTime() + 60 * 60 * 1000);
...
export async function loadDailyActiveUsersSplit(tenancy: Tenancy, now: Date, includeAnonymous: boolean): Promise<ActivitySplit> {
  return await loadDailyActiveSplitFromClickhouse({ tenancy, now, entity: "user", includeAnonymous });
}

export async function loadDailyActiveTeamsSplit(tenancy: Tenancy, now: Date): Promise<ActivitySplit> {
  return await loadDailyActiveSplitFromClickhouse({ tenancy, now, entity: "team", includeAnonymous: false });
}

export async function loadLoginMethods(tenancy: Tenancy): Promise<{ method: string, count: number }[]> {
...
export async function loadRecentlyActiveUsers(tenancy: Tenancy, includeAnonymous: boolean = false): Promise<UsersCrud["Admin"]["Read"][]> {
  const prisma = await getPrismaClientForTenancy(tenancy);
  const dbUsers = await prisma.$replica().projectUser.findMany({
    where: {
      tenancyId: tenancy.id,
...
export const RECENT_LIST_PAGE_SIZE = 100;
