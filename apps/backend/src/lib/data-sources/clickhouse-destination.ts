...
function columnDefinition(column: DataSourceColumn, isPrimaryKey: boolean): string {
  const baseType = column.clickhouseType;
  // Every non-key column is Nullable regardless of the source's NOT NULL, because
  // a delete tombstone carries only the row's identity — Postgres sends the rest as
  // SQL NULL, and Convex sends nothing but `_id`. A non-Nullable column would coerce
  // those to '' or 0 and, depending on server settings, could reject the whole batch.
...
/** Converts a source value into something ClickHouse's JSONEachRow reader accepts. */
