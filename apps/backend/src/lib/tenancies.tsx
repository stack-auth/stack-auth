import { oldDeprecatedPrismaClient, rawQuery } from "@/prisma-client";
...
export async function tenancyPrismaToCrud(prisma: Prisma.TenancyGetPayload<{}>) {
  ...
  const completeConfig = await rawQuery(oldDeprecatedPrismaClient, getRenderedOrganizationConfigQuery({
...
export async function getTenancy(tenancyId: string) {
  ...
  const prisma = await oldDeprecatedPrismaClient.tenancy.findUnique({
    where: { id: tenancyId },
  });
