...
export const postMigration = async (sql: Sql, ctx: Awaited<ReturnType<typeof preMigration>>) => {
  ...
  // The natural key must fire even though every row carries a NULL in one of
  // the two owner columns (that's what the CHECK guarantees) — this is exactly
  // the case a default nulls-distinct unique index would silently admit, and
  // it's the only database-side arbiter for concurrent read-then-create owner
  // mutations.
  ...
  // A different source is a different logical owner record, so it stays allowed.
  ...
  // Same NULLS NOT DISTINCT rationale as IssueOwner_scope_natural_key above:
  // one canonical subscription row per subject, enforced by the database.
