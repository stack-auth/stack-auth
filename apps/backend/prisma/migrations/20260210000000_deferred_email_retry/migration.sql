...
ALTER TABLE "EmailOutbox"
  ADD COLUMN "failedSendAttemptCount" INTEGER NOT NULL DEFAULT 0,
...
-- (if failedSendAttemptCount is 0, no attempt has failed, so there's nothing to retry)
ALTER TABLE "EmailOutbox"
  ADD CONSTRAINT "EmailOutbox_nextSendRetryAt_requires_failure"
  CHECK ("nextSendRetryAt" IS NULL OR "failedSendAttemptCount" > 0);
...
ALTER TABLE "EmailOutbox"
  ...
  CHECK ("sendAttemptErrors" IS NULL OR "failedSendAttemptCount" > 0);
