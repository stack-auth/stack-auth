-- Keep the 24-hour TV occurrence lookup bounded without sharing a migration
-- transaction budget with another potentially large concurrent build.
-- SINGLE_STATEMENT_SENTINEL
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = current_schema() AND c.relname = 'TvEventOccurrence_occurred_lookup_idx'
  ) AND NOT EXISTS (
    SELECT 1
    FROM pg_index i
    ...
    RAISE EXCEPTION 'TV event occurred-at index exists with an unexpected or invalid definition';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'TV event occurred-at index did not finish with the expected definition';
  END IF;
END
$$;
