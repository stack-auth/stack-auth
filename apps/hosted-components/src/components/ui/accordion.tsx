...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-black/[0.08] dark:border-white/[0.08]", className)}
    {...props}
  />
));
...
>(({ className, children, disabled, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    ...
    >
      {children}
      {!disabled && <ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-200" />}
    </AccordionPrimitive.Trigger>
