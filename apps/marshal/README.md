...
Two authenticated maintenance endpoints drive it, scheduled in `vercel.json` (and by `apps/backend/scripts/run-cron-jobs.ts` locally). They live under `/v1/` and use the ordinary bearer check, so set Vercel's `CRON_SECRET` for this project to `MARSHAL_API_KEY`:
