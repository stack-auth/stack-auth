...
A: Custom analytics events now support the full public replay-link tuple: `browserSessionId`/`browser_session_id`, `sessionReplayId`/`session_replay_id`, and `sessionReplaySegmentId`/`session_replay_segment_id`. Browser-side `trackEvent()` automatically fills all three from the active `SessionRecorder` when replay recording is enabled, queued events can override any of them explicitly, and ClickHouse analytics rows now store `browser_session_id` alongside the replay id and segment id.
...
Q: What are the three analytics data signals?
A: Following the OpenTelemetry standard: **Events** (`analytics_internal.events`) — discrete occurrences like `$page-view`, `$click`, `$error`, `$request`, and custom events. **Metrics** (`analytics_internal.metrics`) — numeric measurements like Web Vitals (LCP/FCP/CLS/INP/TTFB), counters, gauges, histograms. **Spans** (`analytics_internal.spans`) — timed operations forming distributed traces (HTTP requests, DB queries, Redis calls). Each signal has its own ClickHouse table, batch API endpoint, and SDK surface.

Q: How do Web Vitals work?
A: The `web-vitals` npm package is dynamically imported in `EventTracker._setupWebVitalsCapture()`. Each vital (LCP, FCP, CLS, INP, TTFB) is collected via callbacks and emitted as a separate metric record to `POST /api/v1/analytics/metrics/batch`. Metrics are stored in the `analytics_internal.metrics` ClickHouse table with `name`, `value`, `unit`, `kind`, `tags`. Web Vitals are emitted once per page view — debounced 5s after the last metric arrives, or immediately on page hide.
...
Q: How does the Nitro plugin for analytics work?
A: `packages/template/src/integrations/nitro.ts` exports `defineStackNitroPlugin({ serverApp })` which hooks into Nitro's `request`/`afterResponse` lifecycle. On `request` it records start time and captures request headers for auth context. On `afterResponse` it auto-emits a `$request` event (with `method`, `path`, `status`, `duration_ms`) via `_trackAutoCapturedEvent`, extracts user identity from the `x-stack-auth` header, and calls `flushAnalytics()` (using `waitUntil` when available). For custom events in server routes, developers use `stackServerApp.trackEvent()` directly — no separate helper needed. Exported as `@stackframe/js/nitro` (and `@stackframe/stack/nitro`, `@stackframe/react/nitro`). The `$request` event type is allowlisted in both the client SDK (`analytics-events.ts`) and the backend batch route.
