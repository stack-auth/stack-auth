...
Q: What changed when support moved off `ThreadMessage` and email replies?
A: Support/inbox data lives in Prisma `Conversation`, `ConversationMetadata`, `ConversationChannel`, and `ConversationMessage` (see migration `20260420000000_add_conversations`). User-facing REST uses `/api/latest/conversations`; dashboard/internal routes use `/api/latest/internal/conversations`. Internal notes use `internal-note` message types and are not exposed on the public conversation API.
...
Q: How should the dashboard Support page hide the global `PageLayout` header when a conversation is open?
A: `PageLayout` only renders its header block when `title`, `description`, or `actions` are provided (`apps/dashboard/src/app/(main)/(protected)/projects/[projectId]/page-layout.tsx`). On `conversations/page-client.tsx`, omit those props while a conversation is selected so the Conversations title/description/New Conversation chrome disappears. Do not set `fillWidth: true` unless you intentionally want the conversation view to span the full main panel width; the default `PageLayout` max width matches the constrained inbox layout.

Q: How is the Support conversation transcript styled to feel like a real chat?
A: In `apps/dashboard/src/app/(main)/(protected)/projects/[projectId]/conversations/page-client.tsx`, render `conversationDetail.messages` as chat rows: customer (`sender.type === "user"`) messages are left-aligned “bubble” UI with the conversation user avatar; support/agent messages are right-aligned bubbles with a headset icon; internal notes are **not** chat bubbles—use a full-width dashed-border annotation strip with a small pencil column and uppercase “Internal note” label so they read as team metadata, not user-visible messages; status changes are centered compact timeline chips. The transcript sits in a `max-h` scroll region with `scrollbarGutter: stable` so long threads don’t push the composer off-screen. Each row should be `w-full min-w-0` and cap bubble width with `max-w-[min(720px,calc(100%-2.5rem))]` (avatar + `gap-2`) so `max-w` percentages don’t resolve against a shrink-wrapped flex row (which makes long support bubbles look “indented” away from the right edge).

Q: How is the Support conversation view laid out when a thread is open?
A: The transcript is the primary surface: a slim ticket header (customer, subject, **Started from** line from `getConversationStartedFromLine` / primary `entryPoints` row else `source`, status badge, last active) sits above a **plain `div` flex column** (not `DesignCard`) wrapping scrollable messages (`min-h-0 flex-1 overflow-y-auto`) plus `SupportComposer` pinned at the bottom — `DesignCard`’s internal `relative` wrapper does not participate in flex height, so `flex-1` on the scroll region inside it never gets a bounded height and the composer can appear stacked at the top with no message area. Editable assignee, priority, tags, save, and read-only SLA live once in a **Team & SLA** strip: fixed-width `aside` on `lg+` (`matchMedia('(min-width: 1024px)')`), and the same form in a `Sheet` on smaller viewports (header **Team** button). Do not duplicate priority/source in the header when those fields are edited in the sidebar; omit technical/ID panels from this page.
