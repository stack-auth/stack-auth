...
A: For the new Support app work, model support as generic conversations rather than support-specific threads: use `Conversation` for identity/status/source plus assignment/tags/SLA/metadata fields, `ConversationEntryPoint` for channel ingress and adapter/entry-point expansion (`chat`, `email`, `api`, `manual`), and `ConversationMessage` for message history (`message`, `internal-note`, `status-change`) with `channelId` referencing entry points by `(tenancyId, id)`. Keep the dashboard UI under `/projects/[projectId]/conversations` (legacy `/projects/[projectId]/support` redirects there). Dashboard admin APIs stay under `/api/latest/internal/conversations`, while dogfood user-facing support APIs live under `/api/latest/internal/dogfood/support/conversations`.
