...
Q: How should `ProjectUser.signedUpAt` be maintained once it becomes the canonical signup timestamp?
A: Treat `signedUpAt` as the authoritative source for `signed_up_at_millis`, recent-signup heuristics, metrics, and external DB sync. Direct user creation paths should set it immediately, existing rows should be backfilled from `createdAt` in the migration, and anonymous upgrades must write `signedUpAt` before flipping `isAnonymous` to `false` so read paths never observe a signed-up user without a signup timestamp.
...
A: Keep `getOAuthUrl(...)` for direct-navigation flows, but have `StackClientInterface.authorizeOAuth(...)` send `Accept: application/json` and let `apps/backend/src/app/api/latest/auth/oauth/authorize/[provider_id]/route.tsx` check the Accept header via `fullReq.headers["accept"]` to return `{ location }` with HTTP 200 after setting the CSRF cookie. On March 11, 2026, using `fetch(..., { redirect: "manual" })` against the cross-origin localhost backend caused browser-level `Failed to fetch` errors during OAuth sign-in even though `/health` checks passed; the JSON authorize response preserved `TurnstileChallengeRequired` handling and avoided that redirect/CORS failure mode. The original implementation used a custom `stack_oauth_response_format` query param, which was replaced with standard HTTP content negotiation via the Accept header.
