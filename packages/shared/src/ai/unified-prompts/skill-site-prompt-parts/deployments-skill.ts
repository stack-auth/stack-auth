...
export const deploymentsSkillSection = deindent`
  ...
  A \`server\` holds exactly one instance. \`minInstances: 1\` (the default) keeps it up; \`0\` keeps its availability and disk semantics but does not guarantee scale-to-zero billing, because a persistent server has no request-triggered suspend. \`minInstances\` above 0 requires a paid plan for BOTH types — on the Free plan the deploy fails up front naming the offending services, so write \`minInstances: 0\` (note that a \`server\` needs it written out).
