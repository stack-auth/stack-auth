...
export const deploymentsSkillSection = deindent`
  ...
  \`memory\` sets how much memory a service gets: \`"1GB" | "2GB" | "4GB" | "8GB"\` on a \`server\` (default \`"1GB"\`), and \`"512MB" | "1GB" | "2GB" | "4GB" | "8GB"\` on a \`serverless\` (default \`"512MB"\`). Write the size with its unit and that exact capitalization — \`"4gb"\`, \`"4 GB"\` and \`"4Gi"\` are all rejected, and \`Mb\` means megabits. Anything above the default needs a paid plan, and a project may hold at most 32GB across its always-on services at once.

  There is no \`cpu\` setting: CPU is derived from memory, because the platform only offers valid memory/CPU combinations and a separately chosen CPU could name one that does not exist. On a \`server\` the three smaller sizes are SHARED-CORE and burstable — \`"1GB"\` is 0.25 vCPU sustained, \`"2GB"\` is 0.5, \`"4GB"\` is 1 — and \`"8GB"\` is the first size with 2 dedicated cores, so a CPU-bound server wants \`"8GB"\` even when it fits in less memory. On a \`serverless\`, \`"512MB"\`/\`"1GB"\`/\`"2GB"\` get 1 CPU, \`"4GB"\` gets 2, and \`"8GB"\` gets 4.

  CHANGING \`memory\` ON A \`server\` REPLACES ITS MACHINE: the service goes down and comes back (seconds to about a minute), because a VM cannot be resized in place. Its persistent disk survives — the disk outlives the instance — so no data is lost, but an unavailability window is. Resize a stateful server deliberately, not incidentally. A \`serverless\` resize is an ordinary rolling deploy with no downtime. Writing the size a service is ALREADY running at (\`memory: "1GB"\` on a server that has never set one) changes nothing and replaces nothing.

  \`builder\` sits beside \`services\`, not inside one, because one machine builds every service of a deploy: \`builder: { memory: "32GB" }\`, one of \`"8GB" | "16GB" | "32GB"\`. It defaults to \`"8GB"\`, or \`"16GB"\` when the build is auto-detected by Railpack, and a request below what the build shape needs is raised to it rather than refused. Raise it when a build is KILLED for running out of memory or disk — a large monorepo install, or a compiler that wants the whole project in memory. It does not affect what services run on; that is each service's own \`memory\`.
