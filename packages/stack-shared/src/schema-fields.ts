...
export const oauthLegacyIdSchema = yupString().oneOf(allProviders).meta({ openapiField: { description: `OAuth provider ID, one of ${allProviders.map(x => `\`${x}\``).join(', ')}`, exampleValue: 'google' } });
export const oauthLegacyTypeSchema = yupString().oneOf(['shared', 'standard']).meta({ openapiField: { description: 'OAuth provider type, one of shared, standard. "shared" uses Stack shared OAuth keys and it is only meant for development. "standard" uses your own OAuth keys and will show your logo and company name when signing in with the provider.', exampleValue: 'standard' } });
export const oauthTypeSchema = yupString().oneOf(allProviders).meta({ openapiField: { description: `OAuth provider type, one of ${allProviders.map(x => `\`${x}\``).join(', ')}`, exampleValue: 'google' } });
...
export const oauthIsSharedSchema = yupBoolean().meta({ openapiField: { description: 'Whether the OAuth provider is shared. If this is set to `true`, it will use Stack Auth\' shared OAuth keys and it is only meant for development.', exampleValue: true } });
...
export const emailIsSharedSchema = yupBoolean().meta({ openapiField: { description: 'Whether the email provider is shared. If this is set to `true`, it will use Stack shared email provider and it is only meant for development.', exampleValue: true } });
