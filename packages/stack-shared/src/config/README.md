...
- `$Level` incomplete config: The base config overridden with the overrides up to level `$Level`, deeply merged into `configDefaults`
- `$Level` rendered config: A normalized incomplete config with those fields removed that can be overridden by a future override
