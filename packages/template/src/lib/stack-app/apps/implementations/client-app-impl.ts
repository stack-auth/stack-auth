...
export class _StackClientAppImplIncomplete<HasTokenStore extends boolean, ProjectId extends string = string> implements StackClientApp<HasTokenStore, ProjectId> {
  ...
  private readonly _currentUserOAuthProvidersCache = createCacheBySession<[], {
    id: string,
    type: string,
    user_id: string,
    account_id?: string,
    ...
    async (session) => {
      return await this._interface.listCurrentUserOAuthProviders(session);
    }
  );
  ...
  protected _clientOAuthProviderFromCrud(crud: {
    id: string,
    type: string,
    user_id: string,
    account_id?: string,
    ...
    allow_sign_in: boolean,
    allow_connected_accounts: boolean,
  }, session: InternalSession): OAuthProvider {
    const app = this;
    return {
      id: crud.id,
      type: crud.type,
      ...
      async delete() {
        await app._interface.deleteCurrentUserOAuthProvider(crud.id, session);
        await app._currentUserOAuthProvidersCache.refresh([session]);
      },
    };
  }
