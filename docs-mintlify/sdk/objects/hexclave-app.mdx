  ...
  <MethodContent>
    <ContentSection title="Parameters">
      ...
      <ParamField body="analytics" type="object">
        Analytics capture configuration. SDK-managed capture is enabled by default; pass `{ enabled: false }` to disable it entirely, or `{ replays: { enabled: true, captureKeystrokes: true } }` to record session replays with privacy-safe keyboard activity counts. Keystroke capture defaults to disabled and never records key values or typed text.
      </ParamField>
      ...
      <ParamField body="automaticSideEffects" type="boolean">
        Whether construction automatically processes authentication query parameters, accesses browser storage, records analytics or session replays, installs listeners, and mounts development UI. Defaults to `true`. Explicit method calls still work normally when set to `false`.
      </ParamField>

      <ParamField body="devTool" type='boolean | "auto"'>
        Whether to show the [dev tool indicator](/guides/other/dev-tool) in browser-like environments. `"auto"` (the default) shows it only in development; `true` always shows it; `false` never does.
      </ParamField>
    </ContentSection>
  ...
  <MethodContent>
    ...
    <ContentSection title="Parameters">
      <ParamField body="options.or" type='"return-null" | "redirect" | "throw"'>
        What to do if the user is not found. The `"redirect"` option is deprecated on `getUser`;
        use `useUser({ or: "redirect" })` in React, or perform an explicit browser redirect after a
        nullable `getUser()` call.
      </ParamField>
    </ContentSection>
...
<MethodLayout>
  <MethodContent>
    <ContentSection title="Parameters">
      ...
      <ParamField body="automaticSideEffects" type="boolean">
        Whether inherited client-side constructor integrations run automatically. Defaults to `true`. Explicit method calls still work normally when set to `false`.
      </ParamField>
    </ContentSection>
...
<MethodLayout>
  <MethodContent>
    ...
    **Overloads:**
    ...
    <Warning>
      The `or: "redirect"` overload is deprecated on both client and server app objects. The app
      type controls permissions, not whether the call runs in a browser. Use `getUser()` or
      `{ or: "throw" }` for server-side authorization, and `useUser({ or: "redirect" })` in a
