...
This is a detailed reference for the `StackApp` object. If you're looking for a more high-level overview, please read the [respective page in the Concepts section](/docs/concepts/stack-app).
...
Most commonly you get an instance of `StackClientApp` by calling [`useStackApp()`](/docs/sdk/hooks/use-stack-app) in a Client Component.
...
<Info>
This object is not usually constructed directly. More commonly, you would construct a [`StackServerApp`](#stackserverapp) instead, pass it into a [`<StackProvider />`](/docs/components/stack-provider), and then use the `useStackApp()` hook to obtain a `StackClientApp`.

The [setup wizard](/docs/getting-started/setup) does these steps for you, so you don't need to worry about it unless you are manually setting up Stack Auth.
...
<MethodLayout>
  <MethodContent>
    React hook version of `getUser()`. Equivalent to the [`useUser()`](/docs/sdk/hooks/use-user) standalone hook, which is an alias for `useStackApp().useUser()`.
...
Like `StackClientApp`, but with [server permissions](/docs/concepts/stack-app#client-vs-server). Has full read and write access to all users.
