...
<Info>
  ...
  [examples](/docs/concepts/backend-integration).
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Should I use client or server access type?">
    ...
    **Server access type** (`X-Stack-Access-Type: server`) is for your secure backend server. It has full access over all user data using your secret server key (`ssk_...`).
    ...
    For more information, see the concept documentation on [StackApp](/docs/concepts/stack-app#client-vs-server).
...
<Columns cols={3}>
  <Card title="Getting Started Guide" href="/docs/getting-started/setup">
