...
You can wrap local development with [`hexclave dev`](/guides/going-further/cli#hexclave-dev) without renaming packages. The CLI injects both `HEXCLAVE_*` and `STACK_*` credentials into the child process, so legacy `@stackframe/*` code that still reads `STACK_*` keeps working. If you set both names yourself, they must match - the SDK throws if they differ.
