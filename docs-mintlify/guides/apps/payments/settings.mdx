...
Some methods have dependencies — Stack Auth will refuse to save if you enable a method that requires a prerequisite (e.g. Link requires Cards) without also enabling the prerequisite. The dialog explains what's missing and asks you to either enable the prerequisite or disable the dependent method.
