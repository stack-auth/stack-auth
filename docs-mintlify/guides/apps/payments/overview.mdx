...
In the dashboard, Payments is split across **Product Lines**, **Products & Items**, **Customers**, **Transactions**, and **Settings** — for shaping your catalogue, inspecting customers, monitoring activity, and configuring Stripe.
...
<Steps>
  ...
  <Step title="Connect Stripe">
    Open **Payments → Settings** in the dashboard and follow the Stripe Connect onboarding flow. You'll be asked for business details, bank info, and identity verification. Once approved, payments are live.
  </Step>
...
Configure product lines, prices, and included items in **Payments → Products & Items** in the dashboard.
...
The same flow is available from the dashboard under **Payments → Customers** using the **Grant Product** dialog and per-customer item adjustments.
...
These apply to both one-time purchases and subscription renewals. Customize them in **Emails → Templates** in the dashboard.
...
During development, enable **test mode** in **Payments → Settings** in the dashboard. All purchases will be free and no real money is charged — products are granted immediately without going through Stripe.
