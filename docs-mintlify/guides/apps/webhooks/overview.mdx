...
- [`issue.created`](/api/webhooks/observability/issuecreated)
- [`issue.regressed`](/api/webhooks/observability/issueregressed)
- [`issue.resolved`](/api/webhooks/observability/issueresolved)
- [`issue.ignored`](/api/webhooks/observability/issueignored)
- [`issue.merged`](/api/webhooks/observability/issuemerged)

The `issue.*` events come from error tracking and behave differently from the rest — they are deduplicated, throttled, and carry two 64-bit fields as decimal strings. See [Issue Webhooks](/guides/apps/observability/webhooks) before consuming them.
