...
Webhooks keep your backend in sync with Hexclave. When an event occurs in your project - for example a user or team is created, updated, or deleted - Hexclave sends a signed POST to a URL you register.

They notify your server. They are not a job runner, cron, or an API for defining triggers. Subscribe to events, verify the signature, and run your own logic.
