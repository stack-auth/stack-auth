...
Enable the RBAC app first. Defining `rbac.permissions` in config does not install the app:
...
`super_admin` here is a permission ID you choose — it is not a built-in. System team permissions are the ones that start with `$`.

On the user object, `hasPermission(team, permissionId)` is team-scoped; `hasPermission(permissionId)` (no team) is a project permission. Same split for `getPermission` and `usePermission`.
