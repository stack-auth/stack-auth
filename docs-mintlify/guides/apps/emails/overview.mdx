...
`sendEmail` resolves once the email has been queued. If validation fails, the user IDs do not exist, or the project is still using the shared email server, it throws a Hexclave known error with a stable `errorCode`:
    ...
    if (error instanceof Error && "errorCode" in error && typeof error.errorCode === "string") {
      switch (error.errorCode) {
        ...
        case "USER_ID_DOES_NOT_EXIST":
          ...
          break;
        default:
          console.error("Hexclave email error", error.errorCode, error.message);
      }
    }
...
Emails integrate with Hexclave UI components automatically (for example verification, password reset, and magic-link flows). Each of those flows uses a built-in template that you can customize in [Templates](/guides/apps/emails/templates).
