...
In the dashboard, the Emails app is organized into **Sent**, **Drafts**, **Templates**, and **Email Settings** tabs for monitoring delivery, composing campaigns, editing templates, and configuring your mail server.
...
If you've composed an email in the dashboard's draft editor, you can trigger it programmatically. See [Drafts](/guides/dashboard-references/emails/drafts) for the full workflow.
...
<Info>
  `sendEmail` requires a custom email server (SMTP, Resend, or Managed). It cannot be used with the shared development server. Configure one in [Email Settings](/guides/dashboard-references/emails/email-settings).
</Info>
...
Emails integrate with Hexclave UI components automatically (for example verification, password reset, and magic-link flows). Each of those flows uses a built-in template that you can customize in the dashboard under **Emails → Templates**.
