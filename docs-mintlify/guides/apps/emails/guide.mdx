...
**While developing**, leave **Shared** selected. Built-in auth mail (verification, password reset, magic link) already works. Custom `sendEmail` calls also go out on shared — Hexclave wraps them as "dev emails" so recipients know they aren't from your domain yet. Those messages are delivered to real inboxes; if one is missing, check spam and **Emails → Sent** in the dashboard.
...
<Warning>
  Never send marketing content as transactional mail - that can get your domain blacklisted. Never send transactional mail as marketing either: users can unsubscribe, and then they would stop getting receipts and other required mail.
</Warning>
...
**An address that isn't a user yet:**
...
Use `emails` when you have an inbox but no Hexclave user for it (a billing contact, someone who paid without signing up). If they already have an account, pass `userIds` instead.
...
type SendEmailOptions =
  ...
  & ({ userIds: string[] } | { allUsers: true } | { emails: string[] })
