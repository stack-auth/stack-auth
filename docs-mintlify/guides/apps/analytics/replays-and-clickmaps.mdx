...
`observability.traceSampleRate` (default `0.1`) samples **traces** only. Session replays are a separate signal and are **not** sampled by that rate — every eligible session is recorded unless you opt out with `analytics.replays.enabled: false` or exhaust the plan allowance.

A replay can still exist for a session whose trace was dropped. Correlation IDs are still attached, so you can jump from the replay to other signals; the waterfall for that session may simply be missing.
...
These features build on the analytics records available through [Warehouse](/guides/apps/warehouse/overview):
...
- [Warehouse](/guides/apps/warehouse/overview) - the event schema, SQL runner, and Tables view
