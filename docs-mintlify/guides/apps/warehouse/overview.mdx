---
title: "Warehouse"
description: "Browse project data and run read-only ClickHouse SQL against your project's dataset"
sidebarTitle: "Overview"
---

The Warehouse app gives you two ways to explore the project data stored in [ClickHouse](https://clickhouse.com/):
...
Enable Warehouse under **Apps -> Warehouse** in the dashboard. Analytics and Observability remain separate apps that produce data Warehouse can query.
...
| `session_replay_id` | `Nullable(String)` | Linked [session replay](/guides/apps/analytics/replays-and-clickmaps#session-replays), if one exists |
...
Open **Warehouse -> Tables**, then pick a table from the sidebar. It opens on `events` by default. The grid shows every column the table returns, newest rows first.
...
Open **Warehouse -> Queries** to get a SQL editor. Write a query and run it. The results appear in the same grid, with the same sorting, search, and CSV export as Tables.
...
- [Analytics](/guides/apps/analytics/overview) - capture product behavior for paths, replays, and clickmaps.
- [Observability](/guides/apps/observability/overview) - capture errors, logs, traces, and performance data.
