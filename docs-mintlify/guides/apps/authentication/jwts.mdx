...
* Private keys are deterministically derived from your project ID, optional anonymous audience, and the `HEXCLAVE_SERVER_SECRET` environment variable. This means no key material is ever stored in the database.
...
* JWTs have a limited lifetime (default is 10 minutes via `HEXCLAVE_ACCESS_TOKEN_EXPIRATION_TIME`)
