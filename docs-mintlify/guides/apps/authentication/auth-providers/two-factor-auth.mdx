...
3. **Sign-in**: After the first factor succeeds, Hexclave asks for the current TOTP code and then issues the session. TOTP is part of sign-in, not a separate check you can run later on an already signed-in user.

Read `user.isMultiFactorRequired` on the server or client to see whether the user has 2FA enabled. Users enroll and disable 2FA in `<AccountSettings />`.
