...
| `countryCode` | string | Geo-IP ISO-3166-1 alpha-2 country code (e.g. `US`, `DE`). See [Fraud Protection](./fraud-protection). |
| `riskScores.bot` | number (0-100) | Bot-confidence score. See [Fraud Protection](./fraud-protection). |
| `riskScores.free_trial_abuse` | number (0-100) | Free-trial / multi-account abuse score. See [Fraud Protection](./fraud-protection). |
...
Numeric fields (the `riskScores.*` family) also support `<`, `<=`, `>`, and `>=`. Enum-style fields (`countryCode`, `authMethod`, `oauthProvider`) support `in_list("a", "b", ...)`.
