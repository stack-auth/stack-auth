...
Restricted users are signed-in users whose account exists, but has not been granted normal application access yet. Hexclave marks these users with `user.isRestricted === true` and provides a `user.restrictedReason` explaining why.
...
"use client";
import { hexclaveClientApp } from "../src/hexclave/client";
