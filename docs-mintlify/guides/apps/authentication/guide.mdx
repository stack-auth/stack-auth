...
To close sign-up entirely (invite-only / an existing directory), set `auth.allowSignUp` to `false` and create users with the [server API](/api/server/users/create-user). People who already have a Hexclave user can still sign in; a first-time OAuth or password sign-up cannot create a new one. See [Sign-up rules](./sign-up-rules#invite-only-or-an-existing-directory).
...
**OTP or magic link never arrives.** In a development environment, auth mail goes out on Hexclave's shared server to the address you typed. It is wrapped as a Hexclave "dev email." Check spam, then **Emails → Sent** in the dashboard.

**OAuth callback fails after Google (or another provider) returns.** Add your app origin as a trusted domain in the cloud dashboard. Hexclave allows localhost callbacks in development; production must list your domain. Register the provider callback Hexclave documents (for Google: `https://api.hexclave.com/api/v1/auth/oauth/callback/google`). See [Launch checklist](../launch-checklist/overview).

**`useHexclaveApp must be used within a HexclaveProvider`.** Put a single `<HexclaveProvider>` in the root layout and pass your app object into it. Nested layouts should not wrap another provider. The hook only works in a Client Component.
