...
After sign-in, the client SDK finishes the return on your origin and establishes the session. Protect pages with `useUser({ or: "redirect" })` in a Client Component - that both sends unauthenticated visitors to hosted sign-in and records the current URL so they come back here. You do not need to read callback query parameters yourself.
