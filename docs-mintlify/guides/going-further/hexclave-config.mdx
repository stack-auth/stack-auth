...
It is not a `.env` file. Database URLs and other app secrets stay in your own environment. Wrap your start command with [`hexclave dev`](/guides/going-further/cli#hexclave-dev) to receive Hexclave credentials (`HEXCLAVE_PROJECT_ID`, keys, API URL, and the `STACK_*` aliases).
...
| `rbac.defaultPermissions.teamCreator` | `Record<string, true>` | Permissions granted to the user added as the team's creator. Client `createTeam` adds you as creator. Members added later get `teamMember`. |
