...
The Hexclave CLI is published as `@hexclave/cli` and exposes the `hexclave` command. Use it for project setup, branch config workflows, and quick admin scripts.

## Install

Install the CLI globally to use the `hexclave` command from any project:
...
```sh title="Terminal"
STACK_CLI_REFRESH_TOKEN=<refresh-token> hexclave project list
```
...
| --- | --- |
| `hexclave init --mode create --apps authentication,teams` | Create `stack.config.ts` for local config-driven development. |
| `hexclave init --mode create-cloud` | Create a new Hexclave Cloud project and write keys to `.env`. |
| `hexclave init --mode link-config --config-file ./stack.config.ts` | Link setup instructions to an existing local config file. |
| `hexclave init --mode link-cloud --select-project-id <id>` | Write keys for an existing cloud project to `.env`. |
...
```sh title="Terminal"
hexclave project create --display-name "My App"
```

Use `--json` when you want machine-readable output:

```sh title="Terminal"
hexclave --json project list
```
...
```sh title="Terminal"
hexclave --project-id <project-id> config pull --config-file ./stack.config.ts
```
...
```sh title="Terminal"
hexclave --project-id <project-id> config push --config-file ./stack.config.ts
```

`config pull` requires `hexclave login`. `config push` supports either `hexclave login` or `STACK_SECRET_SERVER_KEY`.
...
`hexclave exec` runs JavaScript with a preconfigured `HexclaveServerApp` available as `hexclaveServerApp`.

```sh title="Terminal"
hexclave --project-id <project-id> exec "return await hexclaveServerApp.listUsers()"
```

The JavaScript is executed as an async function. Return values are printed as formatted JSON; `undefined` prints nothing.

<Warning>
  `hexclave exec` has server-level access to the selected project. It requires `hexclave login` and intentionally rejects `STACK_SECRET_SERVER_KEY` auth.
</Warning>
