...
- **Protected** areas of your app (for example middleware on `/app/*` or `getUser({ or: "throw" })` on key server routes).
...
<Tabs>
  <Tab title="hexclave/server.ts">
    ```typescript title="hexclave/server.ts"
    import "server-only";
  ...
  <Tab title="hexclave/client.ts">
    ```typescript title="hexclave/client.ts"
