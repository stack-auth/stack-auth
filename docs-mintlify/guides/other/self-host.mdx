...
```env
STACK_TRUSTED_PROXY=generic
```

Use a provider-specific value instead when that provider is the backend's ingress:

- `STACK_TRUSTED_PROXY=vercel`
- `STACK_TRUSTED_PROXY=cloudflare`
- `STACK_TRUSTED_PROXY=cloudrun`

Proxy trust is intentionally never inferred from forwarding headers. A production-like standalone backend with an HTTPS `NEXT_PUBLIC_STACK_API_URL` refuses to start until one of these modes is configured. Development, tests, and plain-HTTP local deployments do not require it.
