...
The bootstrap starts from Freestyle's public BusyBox image, installs and verifies the Node 24 runtime, captures the private `hexclave-js-node24-v1` snapshot, and deletes the builder VM. Then configure the server:

```env
HEXCLAVE_FREESTYLE_API_KEY=...
HEXCLAVE_FREESTYLE_SNAPSHOT_ID=hexclave-js-node24-v1
```
