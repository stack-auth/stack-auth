...
Sometimes, you want to retrieve the user only if they're signed in, and redirect to the sign-in
page otherwise. In a Client Component, pass `{ or: "redirect" }` to `useUser`; the hook will never
return `null`.
