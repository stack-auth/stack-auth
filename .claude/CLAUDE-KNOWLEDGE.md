...
## Q: How are dashboard reference docs generated and kept in sync with the product nav?
A: Source of truth is `packages/stack-shared/src/dashboard-docs/definitions.ts` (metadata + nav labels) and `packages/stack-shared/src/dashboard-docs/bodies/<appId>/<slug>.md` (prose). Run `pnpm run generate-dashboard-reference-docs` (also in `pnpm codegen` and `dev` watch via `generate-dashboard-reference-docs:watch`) to emit `docs-mintlify/guides/dashboard-references/**/*.mdx`, patch the hidden **Dashboard reference** group in `docs-mintlify/docs.json`, and add legacy `/guides/apps/...` redirects. The generator validates each app's `dashboardNavLabel` against `navigationItems[].displayName` in `apps/dashboard/src/lib/apps-frontend.tsx` by parsing that file (it cannot import the dashboard module from a root script). Use `undocumentedDashboardNavLabels` for nav items without docs yet (e.g. Payments **Payouts**). The script then runs `generate-setup-prompt-docs` so downstream generated docs pick up `docs.json` changes.
