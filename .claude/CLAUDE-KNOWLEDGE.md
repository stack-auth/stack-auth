...
A: `scripts/generate-dashboard-reference-docs.ts` discovers every internal `navigationItems` entry from `apps-frontend.tsx` (all tabs, including Payments **Payouts** — nothing is skipped except external-only nav). Prose comes only from `@dashboardReference <appId>/<slug>` JSDoc on dashboard `page-client.tsx` files (`scripts/lib/extract-dashboard-reference-jsdoc.ts`). Missing JSDoc → placeholder body in generated MDX. Slug overrides for awkward nav labels (e.g. Products & Items → `products-and-items`) live in `scripts/lib/dashboard-reference-docs.ts`. Run via `pnpm codegen` / dev watcher on `apps/dashboard/src/**`.
