...
type ServerUser = 
  ...
  & {
    ...
    createApiKey(options): Promise<UserApiKeyFirstView>; //$stack-link-to:#serverusercreateapikeyoptions
    listApiKeys(): Promise<UserApiKey[]>; //$stack-link-to:#serveruserlistapikeys
    // NEXT_LINE_PLATFORM react-like
     ⤷ useApiKeys(): UserApiKey[]; //$stack-link-to:#serveruseruseapikeys
  };
```
