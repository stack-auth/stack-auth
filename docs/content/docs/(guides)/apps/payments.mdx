...
#### Item Grants and Expiration

Items can have optional expiration dates. When a customer has multiple grants of the same item type with different expiration dates, Stack Auth uses a customer-friendly ledger algorithm:

- **Credit consumption order**: Credits are consumed from the grant that expires soonest first. This ensures customers use expiring credits before touching longer-lived ones.
- **Expiration behavior**: When a grant expires, any remaining quantity is removed from the customer's balance.

For example, if a customer has:
- 50 credits expiring January 31
- 100 credits expiring March 31

When they consume 30 credits, those 30 are deducted from the January grant first, leaving 20 credits (January) + 100 credits (March).

<Tip>
To give new customers a starting balance of items without requiring a purchase, create a $0 product in your product line and grant it programmatically during signup using `grantProduct()`.
</Tip>
