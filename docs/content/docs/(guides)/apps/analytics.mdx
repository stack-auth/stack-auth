...
You can share a direct link to any session replay. Click the **copy link** button in the replay viewer header (next to the settings gear) to copy the URL. Shared replay links open in a standalone view with full-width playback and a "Back to all replays" link.

Shared replay links require admin access—anyone without dashboard permissions cannot view them.
