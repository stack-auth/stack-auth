...
The built-in rule tester lets you simulate sign-up requests to see which rules would trigger and what the outcome would be—without affecting real users.

To open the tester, click **Open tester** in the page header (next to the **Add rule** button).

### Test inputs

The tester shows two fields by default:

- **Email**: The email address to test (e.g., `user@company.com`)
- **Sign-up method**: The authentication method (`Password`, `OTP`, `OAuth`, or `Passkey`)

Expand **Advanced options** to configure additional fields like OAuth provider, country, risk scores, and Turnstile settings.

Click **Run test** to see the results.

### Understanding the results

After running a test, the tester shows an outcome card indicating whether the sign-up would be allowed or rejected. Expand the sections below the outcome for more details:

- **Matched rules**: Rules that matched the test input, showing each rule's name, condition, action type, and whether it was the deciding rule.
- **Resolved context**: How the test input was parsed, including the extracted email domain. Useful for debugging conditions that reference context variables.
