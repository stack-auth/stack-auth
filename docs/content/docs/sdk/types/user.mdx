...
code={`type ServerUser = 
    ...
    & {
      ...
      listTeamsPaginated([options]): Promise<{ items: ServerTeam[]; nextCursor: string | null }>; //$stack-link-to:#serveruserlistteamspaginatedoptions
      // NEXT_LINE_PLATFORM react-like
      ⤷ useTeamsPaginated([options]): { items: ServerTeam[]; nextCursor: string | null }; //$stack-link-to:#serveruseruseteamspaginatedoptions
