...
code={`type ServerUser = 
    ...
    & {
      ...
      listTeamsPaginated([options]): Promise<ServerTeam[] & { nextCursor: string | null }>; //$stack-link-to:#serveruserlistteamspaginatedoptions
      // NEXT_LINE_PLATFORM react-like
      ⤷ useTeamsPaginated([options]): ServerTeam[] & { nextCursor: string | null }; //$stack-link-to:#serveruseruseteamspaginatedoptions
...
<CollapsibleTypesSection type="serverUser" property="listTeamsPaginated" signature="[options]" defaultOpen={false}>
  <MethodLayout>
    <MethodContent>
...
{/* IF_PLATFORM react-like */}
<CollapsibleTypesSection type="serverUser" property="useTeamsPaginated" signature="[options]" defaultOpen={false}>
  <MethodLayout>
    <MethodContent>
      <Markdown src="../../snippets/use-on-server-callout.mdx" />

      Functionally equivalent to [`listTeamsPaginated()`](#serveruserlistteamspaginatedoptions), but as a React hook.
    </MethodContent>
    <MethodAside title="Signature">
...
<ClickableTableOfContents title="CurrentServerUser Table of Contents" code={`type CurrentServerUser =
