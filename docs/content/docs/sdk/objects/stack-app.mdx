...
## Table of Contents
  <ClickableTableOfContents code={`type StackServerApp =
    ...
    & {
      ...
      listTeamsPaginated([options]): Promise<ServerTeam[] & { nextCursor: string | null }>; //$stack-link-to:#stackserverapplistteamspaginatedoptions
      // NEXT_LINE_PLATFORM react-like
      ⤷ useTeamsPaginated([options]): ServerTeam[] & { nextCursor: string | null }; //$stack-link-to:#stackserverappuseteamspaginatedoptions
...
**Returns:** `Promise<ServerTeam[] & { nextCursor: string | null }>`
  </MethodContent>
  <MethodAside>
    <AsideSection title="Signature">
    ...
    declare function listTeamsPaginated(options?: {
      ...
      desc?: boolean;
      query?: string;
    }): Promise<ServerTeam[] & { nextCursor: string | null }>;
    ```
    </AsideSection>
    <AsideSection title="Examples">
