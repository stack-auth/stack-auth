...
## Table of Contents
  <ClickableTableOfContents code={`type StackClientApp = {
    ...
    signInWithOAuth(provider, [options]): void;  //$stack-link-to:#stackclientappsigninwithoauthprovider
...
<CollapsibleMethodSection method="signInWithOAuth" signature="provider, [options]" appType="StackClientApp" defaultOpen={false}>
  <MethodLayout>
    <MethodContent>
...
**Parameters:**
- `provider` (string) - The OAuth provider type (e.g., "google", "github", "microsoft")
- `options?` (object) - Optional configuration
  - `returnTo?` (string) - URL to redirect to after the OAuth callback completes. If not specified, defaults to the current URL if it contains an `after_auth_return_to` query parameter, otherwise returns to the default post-auth destination.
...
<Tabs defaultValue="basic">
  <TabsList>
    ...
    <TabsTrigger value="with-return-to">With Return URL</TabsTrigger>
  </TabsList>
...
```
  ...
  <TabsContent value="with-return-to">
```typescript
